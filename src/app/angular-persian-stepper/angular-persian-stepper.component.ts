...
export class AngularPersianStepperComponent implements OnInit {
  ...
  buildStepContents(): void {
    ...
    for (let index = 0; index < refrenceGrowFactor; index++) {
      ...
      if (growthFactor === 0) {
        // (content.parentNode as HTMLElement).style.height = '50%';
        ...
        // const height = ((refrenceGrowFactor - growthFactor) * 5).toString() + '%';
        const width = (100 - growthFactor * 7) + '%';
        // (content.parentNode as HTMLElement).style.height = height;
